...
async function main() {
  ...
  for (const scriptName of pullScripts) {
    ...
    console.log(`→ Running ${scriptName} for vendor "${vendor}" → ${outputDir}`);

    const proc = Bun.spawn(
      ["bun", scriptPath, "--output", outputDir],
      {
        cwd: rootDir,
        stdout: "inherit",
        stderr: "inherit",
  ...
  if (failures.length > 0) {
    console.error(`Some vendor docs scripts failed (codes: ${failures.join(", ")}).`);
    process.exit(1);
  }
