...
const SKIP_SUFFIXES = [".png", ".jpg", ".jpeg", ".gif", ".svg", ".webp", ".ico", ".pdf", ".zip", ".json", ".map", ".woff", ".woff2"];
...
function keyToPage(key: string, outputRoot: string): Page | null {
  ...
  if (lowered.startsWith("blog") || lowered.includes("blog_") || lowered.includes("4koma") || lowered.includes("playground")) {
    return null;
  }
...
async function fetchHtml(baseUrl: string, path: string): Promise<string | null> {
...
function extractMain(html: string): string {
  const mainMatch = html.match(/<div[^>]*class="[^"]*vp-doc[^"]*"[^>]*>[\s\S]*?<\/div>/i);
...
function htmlToMarkdown(rawHtml: string): string {
  ...
  html = html.replace(/<h1[^>]*>([\s\S]*?)<\/h1>/gi, (_, t) => `# ${cleanupText(t)}\n\n`);
  html = html.replace(/<h2[^>]*>([\s\S]*?)<\/h2>/gi, (_, t) => `## ${cleanupText(t)}\n\n`);
  html = html.replace(/<h3[^>]*>([\s\S]*?)<\/h3>/gi, (_, t) => `### ${cleanupText(t)}\n\n`);
  html = html.replace(/<h4[^>]*>([\s\S]*?)<\/h4>/gi, (_, t) => `#### ${cleanupText(t)}\n\n`);

  html = html.replace(/<pre[^>]*><code[^>]*>([\s\S]*?)<\/code><\/pre>/gi, (_, t) => "```\n" + decodeHtml(t.trim()) + "\n```\n\n");
  html = html.replace(/<code[^>]*>([\s\S]*?)<\/code>/gi, (_, t) => "`" + cleanupInline(decodeHtml(t)) + "`");

  html = html.replace(/<p[^>]*>([\s\S]*?)<\/p>/gi, (_, t) => `${cleanupText(t)}\n\n`);
  html = html.replace(/<br\s*\/?>/gi, "\n");

  html = html.replace(/<li[^>]*>([\s\S]*?)<\/li>/gi, (_, t) => `- ${cleanupText(t)}\n`);
  html = html.replace(/<\/ul>/gi, "\n");

  html = html.replace(/<a\s+[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi, (_, href, text) => `[${cleanupInline(text)}](${href})`);
...
async function pullDocs(options: Options) {
  const hashmap = (await fetchJson(new URL(HASHMAP_PATH, options.baseUrl).toString())) as Record<string, string>;
  const pages: Page[] = [];
  ...
  async function worker(page: Page) {
    try {
      const md = (await fetchMd(options.baseUrl, page.path)) ?? (await fetchHtml(options.baseUrl, page.path));
      if (!md) {
        skipped++;
