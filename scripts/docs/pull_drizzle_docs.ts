...
async function listDocsFromRepo(options: Options): Promise<Page[]> {
  ...
  const tree = (data as any).tree as { path: string; type: string }[];
...
async function pullDocs(options: Options) {
  ...
  console.log(`Discovered ${pages.length} docs files from repo ${options.repo} @ ${options.branch} (prefix: ${options.docsPrefix}).`);
